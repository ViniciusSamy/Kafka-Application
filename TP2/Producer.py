...
def routine(name, repeats, servers, topic, partition_control, partition_content ):

    for i in range(repeats):
        print("-------CREATE-CONNECTION------")
        w1 = Writter(servers, topic, "Pub-Carlos", "Sub-Carlos", partition_control, partition_content)
        w1.print()

        print("-------REQUESTING-OBJECT-------")
        success, msg = w1.request()
        print(msg)
        if not success:
            sys.exit()
